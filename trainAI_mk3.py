...
def train_ai(packet):
    ...
    while run:
        if game.whitesTurn:
            candidateMoves = []
            worstCandidate = 0
            worstCandidateOutput = -1.7976931348623157e+308
            for moves in gameInfo.possibleMoves:
                for move in moves:
                    output1 = net1.activate(game.try_move(move))[0]
                    if output1 > worstCandidateOutput or len(candidateMoves) < 3:
                        candidateMoves.append(move)
                        if len(candidateMoves) > 3:
                            candidateMoves.pop(worstCandidate)
                        worstCandidateOutput = 1.7976931348623157e+308
                        for i in range(len(candidateMoves)):
                            candidateOutput = net1.activate(game.try_move(candidateMoves[i]))[0]
                            if worstCandidateOutput > candidateOutput:
                                worstCandidate = i
                                worstCandidateOutput = candidateOutput
            candidateEvals = []
            for move in candidateMoves:
                fakeGame = deepcopy(game)
                fakeGame.make_move(move)
                fakeGameInfo = fakeGame.loop(lastMoves)
                worstEnemyOutput = 1.7976931348623157e+308
                for enemyMoves in fakeGameInfo.possibleMoves:
                    for enemyMove in enemyMoves:
                        enemyOutput = net1.activate(fakeGame.try_move(enemyMove))[0]
                        if enemyOutput < worstEnemyOutput:
                            worstEnemyOutput = enemyOutput
                candidateEvals.append(worstEnemyOutput)
            bestCandidateEval = 0
            for i in len(candidateMoves):
                if candidateEvals[i] > candidateEvals[bestCandidateEval]:
                    bestCandidateEval = i
            bestMove = candidateMoves[bestCandidateEval]
            ...
            for moves in gameInfo.possibleMoves:
                for move in moves:
                    output1 = net2.activate(game.try_move(move))[0]
                    if output1 < worstCandidateOutput or len(candidateMoves) < 3:
                        candidateMoves.append(move)
                        if len(candidateMoves) > 3:
                            candidateMoves.pop(worstCandidate)
                        worstCandidateOutput = -1.7976931348623157e+308
                        for i in range(len(candidateMoves)):
                            candidateOutput = net2.activate(game.try_move(candidateMoves[i]))[0]
                            if worstCandidateOutput < candidateOutput:
                                worstCandidate = i
                                worstCandidateOutput = candidateOutput
            candidateEvals = []
            for move in candidateMoves:
                ...
                for enemyMoves in fakeGameInfo.possibleMoves:
                    for enemyMove in enemyMoves:
                        enemyOutput = net2.activate(fakeGame.try_move(enemyMove))[0]
                        if enemyOutput > worstEnemyOutput:
                            worstEnemyOutput = enemyOutput
                candidateEvals.append(worstEnemyOutput)
            bestCandidateEval = 0
        ...
        if gameInfo.gameStatus == 'B' or gameInfo.gameStatus == 'W' or gameInfo.gameStatus == 'D':
            return calculate_fitness(genome1, genome2, gameInfo, genome1ID, genome2ID)

def calculate_fitness(genome1, genome2, gameInfo, genome1ID, genome2ID):
            ...
            for j in range(len(genomes[i+1:])):
                ...
                if random.random() < GAMESPLAYEDFRACTION:
                    if random.random() < 0.5:
                        ...
                        fitnesses.append(executor.submit(train_ai, (genomes[j][1], genomes[i][1], config, j, i)))
